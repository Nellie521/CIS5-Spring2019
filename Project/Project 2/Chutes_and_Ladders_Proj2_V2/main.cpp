 ...
 * Created on June 3rd, 2019, 1:58 PM
...
void output (int,int,int,int,string,string);    //Output results function
void outArry (unsigned int [],int,int);
...
int main(int argc, char** argv) {
    ...
    int nPlayers=intro(nPlayer,x);  //Function for determining nPlayer
    unsigned int array[SIZE];   //Array for board
...
//Start of game - Inputting nPlayer
int intro (int nPlayer,int  &x) {
...
//One player Chutes and Ladders Game
...
//Two-player Chutes and Ladders Game
    ...
    for (int i=1;i<=nGames;i++) {
        ...
        if (win1==true||win2==true) {
            if (win1==true) {
                ...
                //Output number of wins for each player
                if (i==nGames) {
                    cout<<left<<setw(13)<<name1<<"wins = "<<setw(3)<<cwin1<<endl;
...
//Boolean function to determine win
...
//Output an array - Similar to a board
void outArry (unsigned int array[],int SIZE,int perLine) {
    int i=100;
    array[i]=100;
    for (i=100;i>0;i--) {
        if (i<91) {
